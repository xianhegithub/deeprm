...
def get_traj_worker(pg_learner, env, pa, result):
    ...
    for i in range(pa.num_epis_per_batch):
...
def launch(pa, pg_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    for loop_epi_idx in range(pa.num_epis):

        print ("-prepare for env-", loop_epi_idx)

        env = environment_xh.Env(pa, nw_len_epis=nw_len_epis, nw_size_epis=nw_size_epis,
                              render=False, repre=repre, end=end)
        env.epi_idx = loop_epi_idx
        envs.append(env)

    for batch_idx in range(pa.batch_size + 1):  # last worker for updating the parameters

        print ("-prepare for worker-", batch_idx)
    ...
    for iteration in tqdm(range(1, pa.num_epochs)):

        ps = []  # threads
        manager = Manager()  # managing return results
        manager_result = manager.list([])

        epi_indices = np.arange(pa.num_epis)
        np.random.shuffle(epi_indices)
        ...
        for loop_epi_idx in tqdm(range(pa.num_epis)):
            ...
            rand_epi_idx = epi_indices[loop_epi_idx]
            p = Process(target=get_traj_worker,
                        args=(pg_learners[epi_counter], envs[rand_epi_idx], pa, manager_result, ))
            ps.append(p)

            epi_counter += 1

            if epi_counter >= pa.batch_size or loop_epi_idx == pa.num_epis - 1:

                print (loop_epi_idx, "out of", pa.num_epis)
...
def main():
    ...
    #pg_resume = None
    pg_resume = 'data/pg_su_net_file_20.pkl'
