...
def script_usage():
    print('--exp_type <type of experiment> \n'
          ...
          '--num_steps_in_epi <simulation length> \n'
          '--num_epis <number of examples> \n'
          '--num_epis_per_batch <rough number of samples in one batch update> \n'
...
def main():
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            pa.num_nw = int(arg)
        elif opt in ("-s", "--num_steps_in_epi"):
            pa.num_steps_in_epi = int(arg)
        elif opt in ("-n", "--num_epis"):
            pa.num_epis = int(arg)
        elif opt in ("-sp", "--num_epis_per_batch"):
            pa.num_epis_per_batch = int(arg)
...
# example running parameter
    ...
    if type_exp == 'pg_su':
        ...
        slow_down_cdf_xh.launch(pa, pg_resume, render, True)
    # elif type_exp == 'q_re':
